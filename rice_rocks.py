...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
            #update center based on age
            centerage = [self.image_center[0] * (self.age + 1), self.image_center[1]]
            canvas.draw_image(self.image, centerage, self.image_size, 
...
# helper function to update and draw each sprite in a group
def process_sprite_group(spriteset, canvas):
    for sprite in list(spriteset):
        sprite.draw(canvas)
        sprite.update()
        if sprite.update():
...
def draw(canvas):
    ...
    # update lives if ship collides with rocks
    if group_collide(rock_group, my_ship):
        lives -= 1
        
    #restart game when lives are 0
    if lives == 0:
        started = False
        rock_group = set([])
        timer.stop()
    ...
    #update score if missiles collide with rocks:
...
# helper function to check group collision
...
# initialize ship, rock group, missile group and explosion group
